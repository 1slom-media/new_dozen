...
export default class OrderController {
  ...
  public create = async (
    ...
    try {
      ...
      console.log(data, "data create order");
  ...
  public getTotalBalance = async (
    ...
    try {
      const data = await this.orderDao.totalBalance();
  ...
  public addToOrderItems = async (
    ...
    try {
      ...
      console.log(data, "order to items");
  ...
  public createV1 = async (
    ...
    try {
      ...
      console.log(data, "ord data create v1 controller");
  ...
  public getProductsCategory = async (
    ...
    try {
      const data = await this.orderService.getProductsCategory(req.params.id);
  ...
  public getAllOrderStatusSeller = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const data = await this.orderService.getAllOrderStatusSeller(
        String(status),
        page,
        limit,
        startTime,
  ...
  public updateOrderStatusAdmin = async (
    ...
    try {
      ...
      const { status, startTime, endTime, region } = extractQuery(query).sorts;
      ...
      await new Promise(async (resolve, reject) => {
        for await (let order of orders) {
          await this.orderService.updateOrderAdmin(order._id?.toString(), {
            status: newStatus,
          });
        }
        resolve("success");
  ...
  public getWeekOrders = async (
    ...
    try {
      ...
      const data = await this.orderService.getWeekOrders(nowDate, lastDate);
  ...
  public getRegionCountOrder = async (
    ...
    try {
      ...
      const data = await this.orderService.getRegionCountOrder(Number(number));
  ...
  public updateOrderAdminManyByStatus = async (
    ...
    try {
      const { oldStatus, newStatus }: UpadetOrderAdminManyByStatus = req.body;
      const orders = await this.orderService.getOrdersIdWithStatus(oldStatus);
      await new Promise(async (resolve, reject) => {
        for await (let order of orders) {
