...
export default class OrderDao {
  async create(orderData: IOrder) {
    ...
    await order.save().catch((err) => (message = err.message.split(": ")[2]));
  ...
  async getAllReferalOrders(
    ...
    return {
      orders,
      countPage: countPage[0] ? Math.ceil(countPage[0]["Total"] / limit) : 0,
    };
  }
  ...
  async getOrderWithNameSeller(userId?: string) {
    if (!userId) {
    ...
    const result = await OrderModel.aggregate([
      ...
      {
        $group: {
          _id: "$status", // status bo'yicha guruhlash
          count: { $sum: 1 }, // har bir status uchun count
        },
      },
      {
        $project: {
          status: "$_id", // guruhlangan statusni olish
          count: 1, // count ni olish
          _id: 0, // _id maydonini yashirish
        },
      },
      { $sort: { status: 1 } }, // status bo'yicha tartiblash (ixtiyoriy)
    ]);
